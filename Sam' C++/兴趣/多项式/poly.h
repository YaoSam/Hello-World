...
g.D()							//.D() 返回导数。也就是一个多项式。
*/
const int MAX = 12;	//代表多项式的最高次数。用来设定数组的大小。当然一般会开得大一点。
#define re(i,n)		for(int i=0;i<=n;i++) //从0到n。
class Fx
{
...
{
	degree = -1;
	if (a0 != 0)		degree = 0; coef[0] = a0;
	if (a1 != 0)		degree = 1, coef[1] = a1;
	if (a2!= 0)		degree = 2, coef[2] = a2;
	if (a3!= 0)		degree = 3, coef[3] = a3;
	if (a4!= 0)		degree = 4, coef[4] = a4;
	if (a5!= 0)		degree = 5, coef[5] = a5;
	if (a6!= 0)		degree = 6, coef[6] = a6;
	if (a7!= 0)		degree = 7, coef[7] = a7;
}
...
Fx Fx::operator +(Fx const &other) const
{
	...
	re(i, M)
	{
		...
		if (temp.coef[i] != 0) temp.degree = i;		//确认准确阶数。
	}
	return temp;
}
Fx Fx::operator -(Fx const &other) const
{
	...
	re(i, M)
	{
		...
		if (temp.coef[i] != 0) temp.degree = i;
	}
	return temp;
}
Fx Fx::operator *(Fx const &other) const
